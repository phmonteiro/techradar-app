...
const ReferenceForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchReference = async () => {
      try {
        const token = localStorage.getItem('authToken');
        const response = await axios.get(`http://localhost:3000/api/admin/references/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        ...
        await axios.put(`http://localhost:3000/api/admin/references/${id}`, form, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setSuccess('Reference updated successfully!');
      } else {
        // Create new reference
        await axios.post('http://localhost:3000/api/admin/references', form, {
          headers: { Authorization: `Bearer ${token}` }
        });
