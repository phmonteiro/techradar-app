...
const TechnologiesManagement = () => {
  ...
  const fetchTechnologies = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:3000/api/admin/technologies?page=${pagination.page}&limit=${pagination.limit}&search=${search}`,
        {
          headers: {
  ...
  const confirmDelete = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:3000/api/admin/technologies/${techToDelete.Label}`,
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
