...
const TrendsManagement = () => {
  ...
  const fetchTrends = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:3000/api/admin/trends?page=${pagination.page}&limit=${pagination.limit}&search=${search}`,
        {
          headers: {
  ...
  const confirmDelete = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:3000/api/admin/trends/${trendToDelete.Label}`,
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
