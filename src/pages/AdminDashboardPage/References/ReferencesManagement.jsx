...
const ReferencesManagement = () => {
  ...
  const fetchReferences = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:3000/api/admin/references?page=${pagination.page}&limit=${pagination.limit}&search=${search}`,
        {
          headers: {
  ...
  const confirmDelete = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:3000/api/admin/references/${referenceToDelete.Id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
